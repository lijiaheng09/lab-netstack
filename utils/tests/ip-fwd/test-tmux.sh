set -x
export PROJ=`realpath $PWD/../../..`
export TESTS=$PROJ/utils/tests
export TOOLS=$PROJ/build/tools

cd $PROJ/utils/vnetUtils/examples
sudo ./makeVNet <example.txt
cd ../helper

export veth1_2=`sudo ./execNS ns1 ifconfig | $TESTS/getEtherAddr.py veth1-2`
export veth3_2=`sudo ./execNS ns3 ifconfig | $TESTS/getEtherAddr.py veth3-2`
